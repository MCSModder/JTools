...
public class JSay : Say
{
    ...
    /// <param name="type">对话类型</param>
    /// <returns>JSay 对象</returns>
    public JSay Create(int id, string text, string text2, Func<bool> evaluate,
    ...
    /// <param name="type">对话类型</param>
    /// <returns>JSay 对象</returns>
    public JSay Create(Avatar avatar, string text, string text2, Func<bool> evaluate,
        StartFight.MonstarType type = StartFight.MonstarType.Normal)
    {
        SetAvatar((int)avatar, type);
        _text1 = text;
    ...
    {
        var man = PlayerEx.Player.Sex == 1;
        storyText = storyText.Replace("<公子>", man ? "公子" : "姑娘");
        storyText = storyText.Replace("<少爷>", man ? "少爷" : "小姐");
        storyText = storyText.Replace("<兄弟>", man ? "兄弟" : "姑娘");
        storyText = storyText.Replace("<老爷>", man ? "老爷" : "夫人");
        storyText = storyText.Replace("<小子>", man ? "小子" : "丫头");
        storyText = storyText.Replace("<师兄>", man ? "师兄" : "师姐");
        storyText = storyText.Replace("<师弟>", man ? "师弟" : "师妹");
        storyText = storyText.Replace("<哥哥>", man ? "哥哥" : "姐姐");
        storyText = storyText.Replace("<弟弟>", man ? "弟弟" : "妹妹");
        storyText = storyText.Replace("<他>", man ? "他" : "她");
        storyText = storyText.Replace("<道友>", $"{PlayerEx.Player.firstName}道友");
        storyText = storyText.Replace("<门派>", Tools.getStr($"menpai{PlayerEx.Player.menPai}"));
    }
